...
namespace checkers
{
	game::game(int fps, std::istream& is, std::ostream& os) : m_event_handler(new event_handler(this)), m_game_state(new game_state(os)), m_fps(fps), m_last_capture_direction(-1), m_is(is), m_os(os), m_log_file("log.txt", std::ios::app)
	{
		assert(s_size % 2 == 0);
		assert(m_fps > 0);
		...
		// bind user input
		if (!m_game_state->is_console_game())
		{
		...
		int dummy = 0;
		m_game_state->set_available_capture(m_evaluator->evaluate(m_game_state->get_current_player()->get_list(), m_board, &dummy, dummy, m_last_capture_direction, &m_to_delete_list, nullptr));
#ifdef _DEBUG
...
#endif
		m_gui->draw_board(*m_player_1->get_list(), *m_player_2->get_list(), m_to_delete_list, m_game_state->get_selected_piece());
	}
	...
	evaluator* game::get_evaluator(void) { return m_evaluator; }
	...
	void game::move_selected_piece(void)
	{
		...
		else // making a move
		{
			...
			{
				m_evaluator->clear_list(&m_p_list_1);
				m_evaluator->clear_list(&m_p_list_2);
			}

			// evaluate current player and check if there is more captures, if not, check for new kings
			int counter = 0;
			m_game_state->set_available_capture(m_evaluator->evaluate(m_game_state->get_current_player()->get_list(), m_board, &counter, counter, m_last_capture_direction, &m_to_delete_list, m_game_state->get_moving_piece()));
#ifdef _DEBUG
			m_os << "main game loop: first evaluation counter: " << counter << std::endl;
#endif

			// exit the combo, if no more captures
			if (m_game_state->get_current_player()->get_combo() && !m_game_state->get_available_capture())
			{
				m_game_state->set_moving_piece(nullptr);
			...
			{
				m_evaluator->clear_list(m_game_state->get_next_player()->get_list());
			}
	...
	void game::loop(void)
	{
		...
		while (m_gui->get_window().isOpen())
		{
			...
			sfml::time elapsed_time = m_gui->get_clock().restart();
	...
	void game::debug_info(std::ostream& os)
	{
		...
		os << "console game: ";
		m_game_state->is_console_game() ? os << "true" << std::endl : os << "false" << std::endl;
		...
		m_game_state->get_available_capture() ? os << "true" << std::endl : os << "false" << std::endl;
		...
		os << "finished game: ";
		m_game_state->check_completion() ? os << "true" << std::endl : os << "false" << std::endl;
		...
		os << "selected: ";
		m_game_state->get_selected() ? os << "true" << std::endl : os << "false" << std::endl;

		os << "selected piece: ";
		if (m_game_state->get_selected_piece())
		{
			os << "sign: " << m_game_state->get_selected_piece()->get_owner()->get_sign() << "; x: " << m_game_state->get_selected_piece()->get_x() << "; y: " << m_game_state->get_selected_piece()->get_y() << "; list of available moves: " << m_game_state->get_selected_piece()->get_av_list()->size();
			os << "; is king: ";
			dynamic_cast<king*>(m_game_state->get_selected_piece()) ? os << "true" << std::endl : os << "false" << std::endl;
		}
		else
		{
			os << "nullptr" << std::endl;
		...
		os << "moving piece: ";
		if (m_game_state->get_moving_piece())
		{
			os << "sign: " << m_game_state->get_moving_piece()->get_owner()->get_sign() << "; x: " << m_game_state->get_moving_piece()->get_x() << "; y: " << m_game_state->get_moving_piece()->get_y() << "; list of available moves: " << m_game_state->get_moving_piece()->get_av_list()->size();
			os << "; is king: ";
			dynamic_cast<king*>(m_game_state->get_moving_piece()) ? os << "true" << std::endl : os << "false" << std::endl;
		}
		else
		{
			os << "nullptr" << std::endl;
