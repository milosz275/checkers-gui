...
namespace checkers
{
	bot::bot(char sign, game* game) : base_player(sign, "Bot Stefan") { m_game = game; m_depth = 2; m_counter_select = 0; m_counter_move = 0; m_saved_move = std::pair<int, int>(std::make_pair(-1, -1)); }

	//bot::bot(const bot& bot) : base_player(bot) { m_game = nullptr; m_depth = bot.m_depth; m_saved_move = bot.m_saved_move; m_counter_select = bot.m_counter_select; m_counter_move = bot.m_counter_move; }

	bot::~bot() {}

	std::pair<int, int> bot::get_coordinates(void)
	{
		assert(m_game->get_game_state()->get_current_player() == this);
	...
	//std::pair<int, int> bot::find_best_move(game_delegate* game_copy, std::pair<int, int>& saved_move, int depth)
	//{
	...
	//	std::list<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> list_of_games;
	...
	//					{
	//						game_delegate* local_copy = new game(*game_copy);
	...
	//	for_each(list_of_games.begin(), list_of_games.end(), [this, i = 0](std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>& game_and_coords) mutable
	//	{
	//		// 
	//		game_delegate* g = std::get<0>(game_and_coords);
	...
	//	for_each(list_of_games.begin(), list_of_games.end(), [this, saved_move, i = 0, &best_score, &best_move](std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>& game_and_coords) mutable
	//		{
	//			// 
	//			game_delegate* g = std::get<0>(game_and_coords);
	//			
	...
	//std::vector<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> bot::find_best_move(game_delegate* game_copy, int depth, int alpha, int beta, bool maximizing_player)
	//{
	...
	//	std::vector<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> best_moves;
	//	int best_score = maximizing_player ? s_min_score : s_max_score;

	//	for (available_move* move : available_moves)
	//	{
	//		game_delegate* next_game = new game(*game_copy);
	//		next_game->make_move(move);

	//		// Recursively evaluate the next move with alpha-beta pruning
	//		std::vector<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> next_moves = find_best_move(next_game, depth - 1, alpha, beta, !maximizing_player);
	...
	{
		return {};
		//if (depth == 0 || current_game->get_game_state()->check_completion())
		//{
		//	std::vector<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> result;
		//	result.emplace_back(current_game, std::make_pair(-1, -1), std::make_pair(-1, -1));
		//	return result;
		//}
		//else if (depth < m_depth)
		//{
		//	current_game->get_game_state()->switch_turn();
		//	current_game->clear_list(&current_game->m_p_list_1);
		//	current_game->clear_list(&current_game->m_p_list_2);
		//	int dummy = 0;
		//	current_game->m_available_capture = current_game->evaluate(current_game->m_game_state->get_current_player()->get_list(), current_game->m_board, &dummy, dummy, current_game->m_last_capture_direction, &current_game->m_to_delete_list, nullptr);
		//}
		...
		//				{
		//					game_delegate* game_copy = new game(*current_game);
		...
		//	all_of(list_of_games.begin(), list_of_games.end(), [this, &max_score, &depth, &alpha, &beta, &best_moves](std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>& game_and_coords)
		//		{
		//			game_delegate* game_copy = std::get<0>(game_and_coords);

		//			std::vector<std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>> next_moves = find_best_move(game_copy, depth - 1, alpha, beta, false);
		...
		//	// iterate over all generated games
		//	all_of(list_of_games.begin(), list_of_games.end(), [this, &min_score, &depth, &alpha, &beta, &best_moves](std::tuple<game_delegate*, std::pair<int, int>, std::pair<int, int>>& game_and_coords)
		//		{
		//			game_delegate* game_copy = std::get<0>(game_and_coords);
	...
	void bot::add_to_game_copy_list(std::list<std::tuple<game*, std::pair<int, int>, std::pair<int, int>>>& list_of_games, game* game_copy, std::pair<int, int>* source_coords, std::pair<int, int>* destination_coords)
	{
		...
		//					int dummy = 0;
		//					local_copy->m_available_capture = local_copy->evaluate(local_copy->m_game_state->get_current_player()->get_list(), local_copy->m_board, &dummy, dummy, local_copy->m_last_capture_direction, &local_copy->m_to_delete_list, local_copy->m_moving_piece);
	...
	//void bot::add_to_game_copy_list(std::list<game_delegate*>& list_of_games, game_delegate* game_copy)
	//{
	//	// make checks
	//	bool at_least_one_capture_available = game_copy->m_available_capture;
	//	if (at_least_one_capture_available)
	...
	/*int minimax(game_delegate* game_copy, int depth, int alpha, int beta, int is_maximizing)
	{
	...
	int bot::minimax(game_delegate* input_game, int depth, int alpha, int beta, int is_maximizing)
	{
